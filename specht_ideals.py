...
def equivariant_specht_hilbert_series(mu: Partition):
    # base cases
    if mu.len() == 1:
        return EquivHilbExpr(0, 1)
    if mu.parts[1] == 1:
        hook_length = mu.len()
        first_part = mu.parts[0]

        eq_hs_num = s**(hook_length - 1) * t**(math.comb(hook_length, 2))
        for i in range(2, hook_length):
            eq_hs_num += (1 - t) * s**(i - 2) * t**(math.comb(i - 1, 2)) * sp.product(1 - t - s + s*t**k, (k, i, hook_length - 1))
        eq_hs_num += (1 - t) * s**(hook_length - 2) * t**(math.comb(hook_length - 1, 2))

        eq_hs_den = (1 - t)**(hook_length - 1)
        for i in range(1, hook_length):
            eq_hs_den *= (1 - s*(sum(t**j for j in range(i))))
        
        eq_hs = EquivHilbExpr(eq_hs_num, eq_hs_den)
        eq_hs -= EquivHilbExpr((1 - t)**hook_length - s**hook_length, (1 - t - s)*(1 - t)**(hook_length - 1))
        for i in range(0, first_part - 1):
            parts = [1]*hook_length
            parts[0] += i
            hs = specht_hilbert_series(Partition(parts))
            eq_hs -= EquivHilbExpr(s**(hook_length + i), 1) * EquivHilbExpr(hs.h, (1 - t)**hs.a)
        eq_hs *= EquivHilbExpr(1, s**mu.sum())
        return eq_hs
...
for p in P.partitions:
    gens = lower_ideal_generators(p, p.len() - 1)
    if len(gens) == 3:
        print(p, gens[0:3])
        for i in range(5):
            parts1 = gens[1].parts.copy()
            parts1[0] += i
            parts2 = gens[2].parts.copy()
            parts2[0] += i
            # print(specht_hilbert_series(Partition(parts1)) - specht_hilbert_series(Partition(parts2)))
        print("*********************************")

P = Partitions(8)
for p in P.partitions:
    covered = p.find_covered_partitions()
    for c in covered:
        if p.len() == c.len():
            if (p.len() == 2) or (p.len() == 3 and p.parts[-1:] == c.parts[-1:]) or (p.len() > 3 and p.parts[-2:] == c.parts[-2:]):
                print("GOOD")
            print(p, c)
            for i in range(5):
                parts1 = p.parts.copy()
                ...
                parts2 = c.parts.copy()
                parts2[0] += i
                print((specht_hilbert_series(Partition(parts1)) - specht_hilbert_series(Partition(parts2))).degree())
            print("*********************************")